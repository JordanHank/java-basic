...
生产者/消费者问题是生产者和消费者在同一时间段内共用同一个存储空间，生产者向空间里存放数据；而消费者取用数据；如果不加以协调可能会造成死锁。

比如：存储空间已满，而生产者占用着它，消费者等着生产者让出空间从而去除产品，生产者等着消费者消费产品，从而向空间中添加产品。互相等待，从而发生死锁。

![生产者/消费者示例图](../_media/multithread/producerConsumerQuestion.png)


## 解决方案

生产者/消费者问题可以通过多线程同步进行解决：
...
![生产者/消费者解决方案结果图](../_media/multithread/pcqResult.png)
