...
public class ObjDeclare {
    ...
    /**
     * 在java中任何事物都可以抽象成类的概念，类是一类事物的统称描述
     * 例如：生活中存在很多车，小汽车、大卡车、火车...这些我们都可以抽象的统称为车
     * 这里车也就是java中类的概念
     *
     * 类的定义：
     *      1、属性（包括静态属性和非静态属性）
     *      2、方法（包括静态方法和非静态方法）
     *      3、构造函数（包括无参构造函数和有参构造函数）
     *      关键字class后面的名称就是类名，一个java文件只允许有一个public的类并且
     *      public类名必须和文件名相同，不然无法编译通过。
     *      类文件的第一行用来声名包 package com.blackj.leran.java.grammer;
     *      在包后面是引用语句  import java.util*;
